...
class SetAlphaBeta(SetGA):
  """Modify a forwarding rule to direct network traffic to a new target."""

  @staticmethod
  def Args(parser):
    utils.ForwardingRulesTargetMutator.Args(
        parser, include_beta_targets=True)
...
SetGA.detailed_help = {
    ...
    'DESCRIPTION': (_DESCRIPTION + """
        When modifying a forwarding rule, exactly one of  ``--target-instance''
        ``--target-pool'', ``--target-vpn-gateway'', and
        ``--target-http-proxy'' must be specified.
        """),
}


SetAlphaBeta.detailed_help = {
    'brief': ('Modify a forwarding rule to direct network traffic to a new '
              'target'),
    'DESCRIPTION': (_DESCRIPTION + """
        When modifying a forwarding rule, exactly one of  ``--target-instance''
        ``--target-pool'', ``--target-http-proxy'', and
        ``--target-https-proxy'' must be specified.
        """),
}

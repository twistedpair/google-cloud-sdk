...
class GetFilterOperands(resource_expr_rewrite.Backend):
  ...
  def Rewrite(self, expression, defaults=None, keys=None):
    ...
    """
    self._keys = keys or {}
    _, operands = super(GetFilterOperands, self).Rewrite(
  ...
  def RewriteAND(self, left, right):
    """AND is good -- combine the left and right sets."""
    if left:
      if right:
        return left | right
      return left
  ...
  def RewriteTerm(self, key, op, operand, key_type):
    ...
    if isinstance(operand, list):
      # key:(....) or key=(...)
      if 'AND' in operand or 'NOT' in operand:
        return None
      return {x for x in operand if x != 'OR'}

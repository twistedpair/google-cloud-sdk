...
class OperationPoller(object):
  ...
  def Poll(self, operation):
    ...
    request = OperationsMessages().CloudresourcemanagerOperationsGetRequest(
        operationsId=OperationNameToId(operation.name))
    latest = OperationsService().Get(request)
...
class OperationFailedException(calliope_exceptions.ToolException):
  ...
  def __init__(self, operation_with_error):
    op_id = OperationNameToId(operation_with_error)

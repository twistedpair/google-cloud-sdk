...
# TODO(vilasj): Convert usage of old % string formatting to new style.
...
def _HashFromFileHandle(file_handle):
  ...
  # TODO(kash): A future optimization could make appcfg.py store a
...
class AppVersionUploader(object):
  ...
  def Begin(self):
    ...
    for path, content_hash in self.files.iteritems():
      ...
      if file_classification.IsErrorFile():
        # TODO(jonmac): Clone error blobs instead of re-uploading them each and
  ...
  def IsServing(self):
    ...
    # TODO(mbellamy): Remove this backwards compatibility hack when possible.
  ...
  def IsEndpointsConfigUpdated(self):
    ...
    if 'updatedDetail2' in result:
      ...
      # TODO(kdeus): Remove this once http://b/15291459 is fixed and pushed to
      # prod. (b/15333622)

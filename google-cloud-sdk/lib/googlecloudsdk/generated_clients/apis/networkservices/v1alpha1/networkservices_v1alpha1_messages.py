...
class HttpFilter(_messages.Message):
  ...
  Fields:
    config: Optional. The configuration needed to enable the HTTP filter. The
      configuration must be JSON formatted and only contain fields defined in
      the protobuf identified in config_type_url. This field will be ignored
      if the `typed_config` field is set.
    configTypeUrl: Optional. The fully qualified versioned proto3 type url
      that the filter expects for its configuration. For example:
      'type.googleapis.com/envoy.config.wasm.v2.WasmService'. This field will
      be ignored if the `typed_config` field is set.
    createTime: Output only. The timestamp when the resource was created.
    description: Optional. A free-text description of the resource. Max length
      1024 characters.
...
class Mesh(_messages.Message):
  ...
  Fields:
    ...
    envoyHeaders: Optional. Determines if envoy will insert internal debug
      headers into upstream requests. Other Envoy headers may still be
      injected. By default, envoy will not insert any debug headers.
    forwardProxyServiceAttachments: Optional. Specifies a list of service
      attachments containing forward proxies facilitating cross-network
      connectivity. Format: projects/{project}/regions/{region}/serviceAttachm
  ...
  forwardProxyServiceAttachments = _messages.StringField(4, repeated=True)

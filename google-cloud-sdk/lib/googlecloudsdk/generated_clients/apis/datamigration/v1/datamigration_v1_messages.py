...
class BadRequest(_messages.Message):
  r"""Describes violations in a client request. This error type focuses on the
  ...
  fieldViolations = _messages.MessageField('FieldViolation', 1, repeated=True)
...
class FieldViolation(_messages.Message):
  r"""A message type used to describe a single bad request field.

  Fields:
    description: A description of why the request element is bad.
    field: A path that leads to a field in the request body. The value will be
      a sequence of dot-separated identifiers that identify a protocol buffer
      field. Consider the following: message CreateContactRequest { message
      EmailAddress { enum Type { TYPE_UNSPECIFIED = 0; HOME = 1; WORK = 2; }
      optional string email = 1; repeated EmailType type = 2; } string
      full_name = 1; repeated EmailAddress email_addresses = 2; } In this
      example, in proto `field` could take one of the following values: *
      `full_name` for a violation in the `full_name` value *
      `email_addresses[1].email` for a violation in the `email` field of the
      first `email_addresses` message * `email_addresses[3].type[2]` for a
      violation in the second `type` value in the third `email_addresses`
      message. In JSON, the same values are represented as: * `fullName` for a
      violation in the `fullName` value * `emailAddresses[1].email` for a
      violation in the `email` field of the first `emailAddresses` message *
      `emailAddresses[3].type[2]` for a violation in the second `type` value
...
class PreconditionFailure(_messages.Message):
  ...
  violations = _messages.MessageField('PreconditionFailureViolation', 1, repeated=True)


class PreconditionFailureViolation(_messages.Message):
...
class QuotaFailure(_messages.Message):
  r"""Describes how a quota check failed. For example if a daily limit was
  ...
  violations = _messages.MessageField('QuotaFailureViolation', 1, repeated=True)


class QuotaFailureViolation(_messages.Message):
  r"""A message type used to describe a single quota violation. For example, a
  daily quota or a custom quota that was exceeded.

  Messages:
    QuotaDimensionsValue: The dimensions of the violated quota. Every non-
      global quota is enforced on a set of dimensions. While quota metric
      defines what to count, the dimensions specify for what aspects the
      counter should be increased. For example, the quota "CPUs per region per
      VM family" enforces a limit on the metric
      "compute.googleapis.com/cpus_per_vm_family" on dimensions "region" and
      "vm_family". And if the violation occurred in region "us-central1" and
  ...
  Fields:
    apiService: The API Service from which the `QuotaFailure.Violation`
      ...
      called API is Kubernetes Engine API (container.googleapis.com), and a
      quota violation occurs in the Kubernetes Engine API itself, this field
      would be "container.googleapis.com". On the other hand, if the quota
      violation occurs when the Kubernetes Engine API creates VMs in the
      Compute Engine API (compute.googleapis.com), this field would be
      "compute.googleapis.com".
    description: A description of how the quota check failed. Clients can use
      this description to find more about the quota configuration in the
      service's public documentation, or find the relevant quota limit to
      adjust through developer console. For example: "Service disabled" or
      "Daily Limit for read operations exceeded".
    futureQuotaValue: The new quota value being rolled out at the time of the
      violation. At the completion of the rollout, this value will be enforced
      in place of quota_value. If no rollout is in progress at the time of the
      violation, this field is not set. For example, if at the time of the
      violation a rollout is in progress changing the number of CPUs quota
      from 10 to 20, 20 would be the value of this field.
    quotaDimensions: The dimensions of the violated quota. Every non-global
      quota is enforced on a set of dimensions. While quota metric defines
      what to count, the dimensions specify for what aspects the counter
      should be increased. For example, the quota "CPUs per region per VM
      family" enforces a limit on the metric
      "compute.googleapis.com/cpus_per_vm_family" on dimensions "region" and
      "vm_family". And if the violation occurred in region "us-central1" and
      for VM family "n1", the quota_dimensions would be, { "region": "us-
      central1", "vm_family": "n1", } When a quota is enforced globally, the
    ...
    quotaMetric: The metric of the violated quota. A quota metric is a named
      ...
      occurs in a service, such as Virtual Machine allocation, one or more
      quota metrics may be affected. For example,
      "compute.googleapis.com/cpus_per_vm_family",
      "storage.googleapis.com/internet_egress_bandwidth".
    quotaValue: The enforced quota value at the time of the `QuotaFailure`.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class QuotaDimensionsValue(_messages.Message):
    r"""The dimensions of the violated quota. Every non-global quota is
    enforced on a set of dimensions. While quota metric defines what to count,
    the dimensions specify for what aspects the counter should be increased.
    For example, the quota "CPUs per region per VM family" enforces a limit on
    the metric "compute.googleapis.com/cpus_per_vm_family" on dimensions
    "region" and "vm_family". And if the violation occurred in region "us-
    central1" and for VM family "n1", the quota_dimensions would be, {
    "region": "us-central1", "vm_family": "n1", } When a quota is enforced
    globally, the quota_dimensions would always be empty.

    Messages:
      AdditionalProperty: An additional property for a QuotaDimensionsValue
        object.

    Fields:
      additionalProperties: Additional properties of type QuotaDimensionsValue
    """

    class AdditionalProperty(_messages.Message):
      r"""An additional property for a QuotaDimensionsValue object.
  ...
  apiService = _messages.StringField(1)
  description = _messages.StringField(2)
  futureQuotaValue = _messages.IntegerField(3)
  quotaDimensions = _messages.MessageField('QuotaDimensionsValue', 4)
  quotaId = _messages.StringField(5)
  quotaMetric = _messages.StringField(6)
  quotaValue = _messages.IntegerField(7)
  subject = _messages.StringField(8)


class RequestInfo(_messages.Message):
  r"""Contains metadata about the request that clients can attach when filing
  a bug or providing other forms of feedback.
...
class RetryInfo(_messages.Message):
  r"""Describes when the clients can retry a failed request. Clients could
  ignore the recommendation here or retry when this information is missing
  from error responses. It's always recommended that clients should use
  exponential backoff when retrying. Clients should wait until `retry_delay`
  amount of time has passed since receiving the error response before
  retrying. If retrying requests also fail, clients should use an exponential
  backoff scheme to gradually increase the delay between retries based on
  `retry_delay`, until either a maximum number of retries have been reached or
  a maximum retry delay cap has been reached.

  Fields:
    retryDelay: Clients should wait at least this long between retrying the
      same request.
  """

  retryDelay = _messages.StringField(1)

...
class Aggregation(_messages.Message):
  ...
  Fields:
    alias: Optional. Optional name of the field to store the result of the
      ...
      COUNT_UP_TO(4) OVER ( ... ); ``` becomes: ``` AGGREGATE COUNT_UP_TO(1)
      AS count_up_to_1, COUNT_UP_TO(2) AS field_1, COUNT_UP_TO(3) AS
      count_up_to_3, COUNT_UP_TO(4) AS field_2 OVER ( ... ); ``` Requires: *
      Must be unique across all aggregation aliases. * Conform to document
...
class Count(_messages.Message):
  ...
  Fields:
    upTo: Optional. Optional constraint on the maximum number of documents to
      ...
      documents to scan, limiting latency and cost. Unspecified is interpreted
      as no bound. High-Level Example: ``` AGGREGATE COUNT_UP_TO(1000) OVER (
      SELECT * FROM k ); ``` Requires: * Must be greater than zero when
      present.
  """

...
class DomainRoutingFormatter(base_formatter.BaseFormatter):
  ...
  def TransformComponentStatus(self, record):
    ...
    for component in self._GetSSLStatuses(resource_components):
  ...
  def _GetSSLStatuses(self, resource_components):
    ...
    for component in ssl_cert_components:
      statuses.append(
          (self._GetDomainFromSSLComponentName(component.get('name')),
  ...
  def _GetDomainFromSSLComponentName(self, name):
    return name.replace('domain-routing-', '').replace(
        '-cert', '').replace('-', '.')

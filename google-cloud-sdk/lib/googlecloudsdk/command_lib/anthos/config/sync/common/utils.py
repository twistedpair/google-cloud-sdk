...
def AssertConfigSyncEnabled(membership):
  """Get Config Sync objects to check if Config Sync is enabled.
  ...
  with Timeout(_KUBECTL_TIMEOUT):
    _, err = RunKubectl([
        'get',
        'rootsyncs.configsync.gke.io,reposyncs.configsync.gke.io,resourcegroups.kpt.dev',
    ])
    timed_out = False
  if timed_out:
    if IsConnectGatewayContext():
      raise exceptions.ConfigSyncError(
          'Timed out getting Config Sync objects. '
          + 'Make sure you have setup Connect Gateway for '
          + membership
      ...
      raise exceptions.ConfigSyncError(
          'Timed out getting Config Sync objects from ' + membership
      )

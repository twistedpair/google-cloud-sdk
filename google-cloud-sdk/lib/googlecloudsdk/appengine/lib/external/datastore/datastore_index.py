...
from googlecloudsdk.appengine.lib.external.googlestorage.onestore.v3 import entity_pb

from googlecloudsdk.appengine.lib.external.api import appinfo
from googlecloudsdk.appengine.lib.external.api import datastore_types
from googlecloudsdk.appengine.lib.external.api import validation
from googlecloudsdk.appengine.lib.external.api import yaml_errors
from googlecloudsdk.appengine.lib.external.api import yaml_object
from googlecloudsdk.appengine.lib.external.datastore import datastore_pb
...
def GetRecommendedIndexProperties(properties):
  ...
  for sub_list in itertools.chain((prefix,), postfix):
    if isinstance(sub_list, (frozenset, set)):
      # Recommend properties in a consistent order a ascending.
      for prop in sorted(sub_list):
        result.append((prop, ASCENDING))
    else:
      # Recommend properties in the given order, with the given direction
      # or ascending if no direction is given.
...
def _MatchPostfix(postfix_props, index_props):
  ...
  for property_group in reversed(postfix_props):
    ...
    if isinstance(property_group, (frozenset, set)):
      ...
      # Otherwise, it is a list of tuples where the direction does matter.
...
def IndexYamlForQuery(kind, ancestor, props):
  ...
  """
  # TODO(user): Support for 'mode'. Use ToYAML() here?

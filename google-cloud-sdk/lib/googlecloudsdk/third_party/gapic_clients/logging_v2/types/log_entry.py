...
class LogEntry(proto.Message):
    ...
    Attributes:
        ...
        labels (Sequence[googlecloudsdk.third_party.gapic_clients.logging_v2.types.LogEntry.LabelsEntry]):
            Optional. A set of user-defined (key, value)
            data that provides additional information about
            the log entry.
            Cloud Logging truncates label keys that exceed
            512 B and label values that exceed 64 KB upon
            their associated log entry being written. The
            truncation is indicated by an ellipsis at the
            end of the character string.
        metadata (google.api.monitored_resource_pb2.MonitoredResourceMetadata):
            Output only. Deprecated. Additional metadata about the
            monitored resource.

            Only ``k8s_container``, ``k8s_pod``, and ``k8s_node``
            MonitoredResources have this field populated for GKE
            versions older than 1.12.6. For GKE versions 1.12.6 and
            above, the ``metadata`` field has been deprecated. The
            Kubernetes pod labels that used to be in
            ``metadata.userLabels`` will now be present in the
            ``labels`` field with a key prefix of ``k8s-pod/``. The
            system labels that were present in the
            ``metadata.systemLabels`` field will no longer be available
            in the log entry.
        operation (googlecloudsdk.third_party.gapic_clients.logging_v2.types.LogEntryOperation):
    ...
    metadata = proto.Field(proto.MESSAGE, number=25,

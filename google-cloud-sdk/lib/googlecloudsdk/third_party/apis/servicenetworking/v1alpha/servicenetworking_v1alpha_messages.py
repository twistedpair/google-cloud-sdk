"""Generated message classes for servicenetworking version v1alpha.

The Google Service Networking API provides automatic management of network
...
class PeerSharedNetworkRequest(_messages.Message):
  """Request to create a peering to a managed service from a shared service
  consumer network to a producer network hosting service resources. This
  method may be invoked repeatedly if peering connection had been disconnected
  on the consumer side. In such a case previously provisioned subnetwork on
  the service producer side may conflict with a newly provisioned subnetworks
  on the consumer side if reserved range had been changed. In such a case
  services would need to be deprovisioned first, or conflicting consumer
  subnetworks removed.

  Fields:
    network: Name of an existing VPC network that will be peered with service
...
class ServicenetworkingOperationsGetRequest(_messages.Message):
  """A ServicenetworkingOperationsGetRequest object.
...
class ServicenetworkingServicesAddSubnetworkRequest(_messages.Message):
  ...
  Fields:
    addSubnetworkRequest: A AddSubnetworkRequest resource to be passed as the
      request body.
    parent: Project and region where a subnetwork will be created. This is a
      'tenant' project in the service producer organization. services/{service
      }/{collection-id}/{resource-id} {collection id} is the cloud resource
      collection type representing the tenant project. Only 'projects' are
      currently supported. {resource id} is the tenant project numeric id:
      '123456'. {service} the name of the peering service, for example
      'service.googleapis.com'. This service must be whitelisted that has
      established the connection with consumer project.
  """
...
class ServicenetworkingServicesPeerRequest(_messages.Message):
  """A ServicenetworkingServicesPeerRequest object.

  Fields:
    name: Provider that needs to be connected. services/{peering service}
      {peering service} is the service managing peering connectivity for
      service provider organization. For Google services that support this
      functionality it is 'servicenetworking.googleapis.com'.
    peerSharedNetworkRequest: A PeerSharedNetworkRequest resource to be passed
      as the request body.
  """

...
class ReachabilityDetails(_messages.Message):
  ...
  class ResultValueValuesEnum(_messages.Enum):
    ...
    Values:
      ...
      UNKNOWN: The reachability could not be determined. Possible reasons are:
        * Analysis is aborted due to permission error. User does not have read
        permission to the projects listed in the test. * Analysis is aborted
        due to internal errors. * Analysis is partially complete based on
        configurations where the user   has permission.   Final state

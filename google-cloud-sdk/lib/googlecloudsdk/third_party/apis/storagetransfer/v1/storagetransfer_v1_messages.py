...
class AwsS3Data(_messages.Message):
  ...
  Fields:
    awsAccessKey: Required. Input only. AWS access key used to sign the API
      requests to the AWS S3 bucket. Permissions on the bucket must be granted
      to the access ID of the AWS access key. For information on our data
      retention policy for user credentials, see [User credentials](data-
...
class AzureBlobStorageData(_messages.Message):
  ...
  Fields:
    azureCredentials: Required. Input only. Credentials used to authenticate
      ...
      user credentials, see [User credentials](data-retention#user-
      credentials).
...
class ObjectConditions(_messages.Message):
  ...
  only to Cloud Data Sources such as S3, Azure, and Cloud Storage. The "last
  ...
  Fields:
    excludePrefixes: `exclude_prefixes` must follow the requirements described
      for include_prefixes. The max size of `exclude_prefixes` is 1000.
    includePrefixes: If `include_prefixes` is specified, objects that satisfy
      the object conditions must have names that start with one of the
      `include_prefixes` and that do not start with any of the
      exclude_prefixes. If `include_prefixes` is not specified, all objects
      except those that have names starting with one of the `exclude_prefixes`
      must satisfy the object conditions. Requirements: * Each include-prefix
      and exclude-prefix can contain any sequence of Unicode characters, to a
      ...
      expression matching are not supported. * Each include-prefix and
      exclude-prefix must omit the leading slash. For example, to include the
      `requests.gz` object in a transfer from `s3://my-aws-
      bucket/logs/y=2015/requests.gz`, specify the include prefix as
      `logs/y=2015/requests.gz`. * None of the include-prefix or the exclude-
      prefix values can be empty, if specified. * Each include-prefix must
      include a distinct portion of the object namespace. No include-prefix
      may be a prefix of another include-prefix. * Each exclude-prefix must
      exclude a distinct portion of the object namespace. No exclude-prefix
      may be a prefix of another exclude-prefix. * If `include_prefixes` is
    ...
    lastModifiedBefore: If specified, only objects with a "last modification
      ...
      modification time" will be transferred.
...
class Schedule(_messages.Message):
  ...
  Fields:
    ...
    scheduleEndDate: The last day a transfer runs. Date boundaries are
      determined relative to UTC time. A job will run once per 24 hours within
      the following guidelines: * If `schedule_end_date` and
      schedule_start_date are the same and in the future relative to UTC, the
      transfer is executed only one time. * If `schedule_end_date` is later
      than `schedule_start_date` and `schedule_end_date` is in the future
    ...
    scheduleStartDate: Required. The start date of a transfer. Date boundaries
      ...
      **Note:** When starting jobs at or near midnight UTC it is possible that
      ...
      outbound request on June 1 one millisecond prior to midnight UTC and the
      Storage Transfer Service server receives the request on June 2, then it
      will create a TransferJob with `schedule_start_date` set to June 2 and a
...
class TransferJob(_messages.Message):
  ...
  Fields:
    ...
    name: A unique name (within the transfer project) assigned when the job is
      ...
      error. This name must start with `"transferJobs/"` prefix and end with a
      letter or a number, and should be no more than 128 characters. Example:
...
class TransferOptions(_messages.Message):
  ...
  Fields:
    ...
    overwriteObjectsAlreadyExistingInSink: When to overwrite objects that
      ...
      sink whose name matches an object in the source will be overwritten with
      the source object.
  """

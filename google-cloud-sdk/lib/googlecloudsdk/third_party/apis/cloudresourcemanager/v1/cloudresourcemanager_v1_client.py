...
class CloudresourcemanagerV1(base_api.BaseApiClient):
  ...
  class ProjectsService(base_api.BaseApiService):
    ...
    def TestIamPermissions(self, request, global_params=None):
      r"""Returns permissions that a caller has on the specified Project. For additional information about `resource` (e.g. my-project-id) structure and identification, see [Resource Names](/apis/design/resource_names). There are no permissions required for making this API call.

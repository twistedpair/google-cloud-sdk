...
class ComposerProjectsLocationsEnvironmentsPatchRequest(_messages.Message):
  ...
  Fields:
    ...
    updateMask: Required. A comma-separated list of paths, relative to
      ...
      following fields can be updated: *Mask* *Purpose*
      config.softwareConfig.pypiPackages Replace all custom custom PyPI
      packages. If a replacement package map is not included in `environment`,
      all custom PyPI packages are cleared. It is an error to provide both

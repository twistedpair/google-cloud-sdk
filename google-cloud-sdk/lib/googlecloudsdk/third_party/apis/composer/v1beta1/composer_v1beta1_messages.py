...
Manages Airflow environments on Google Cloud Platform.
"""
...
class ComposerProjectsLocationsEnvironmentsPatchRequest(_messages.Message):
  ...
  Fields:
    ...
    updateMask: Required. A comma-separated list of paths, relative to
      ...
      <td>config.softwareConfig.envVariables.<var>varName</var></td>  <td>Set
      the environment variable named <var>varName></var>, while  preserving
      other environment variables. To delete the environment  variable,
      include it in `updateMask` and omit its mapping in
      `environment.config.softwareConfig.envVariables`. It is an error to
      provide both a mask of this form and the
      "config.softwareConfig.envVariables" mask.</td>  </tr>  </tbody>
      </table>
  """
...
class NodeConfig(_messages.Message):
  ...
  Fields:
    ...
    oauthScopes: Optional. The set of Google API scopes to be made available
      on all of the node VMs under the "default" service account. Cannot be
      updated.

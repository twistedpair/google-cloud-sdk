...
class Variable(_messages.Message):
  ...
  int x = 5      { name: "x", value: "5" }  // Captured variable  2) A
  compound object:      struct T {         int m1;         int m2;     };
  T x = { 3, 7 };      {  // Captured variable         name: "x",
  members { name: "m1", value: "3" },         members { name: "m2", value: "7"
  }     }  3) A pointer where the pointee was captured:      T x = { 3, 7 };
  T* p = &x;      {  // Captured variable          name: "p",          value:
  "0x00500500",          members { name: "m1", value: "3" },          members
  { name: "m2", value: "7" }     }  4) A pointer where the pointee was not
  captured:      T* p = new T;      {  // Captured variable         name: "p",
  value: "0x00400400"         status { is_error: true, description { format:
  "unavailable" } }     }  The status should describe the reason for the
  missing value, such as `<optimized out>`, `<inaccessible>`, `<pointers limit
  reached>`.  Note that a null pointer should not have members.  5) An unnamed
  value:      int* p = new int(7);      // Captured variable     { name: "p",
  value: "0x00500500", members { value: "7" } }  6) An unnamed pointer where
  the pointee was not captured:      int* p = new int(7);     int** pp = &p;
  {  // Captured variable         name: "pp",         value: "0x00500500",
  members { value: "0x00400400", members { value: "<unavailable>" } }     }
  To optimize computation, memory and network traffic, variables that repeat
  in the output multiple times can be stored once in a shared variable table
  and be referenced using the `var_table_index` field.  The variables stored
  in the shared table are nameless and are essentially a partition of the
  ...
  variable table, the following variables:      T x = { 3, 7 };     T* p = &x;
  T& r = x;      { name: "x", var_table_index: 3 }  // Captured variables
  { name: "p", value "0x00500500", var_table_index: 3 }     { name: "r",
  var_table_index: 3 }      {  // Shared variable table entry #3:
  members { name: "m1", value: "3" },         members { name: "m2", value: "7"
  ...
  variable to be shared between pointers and references.

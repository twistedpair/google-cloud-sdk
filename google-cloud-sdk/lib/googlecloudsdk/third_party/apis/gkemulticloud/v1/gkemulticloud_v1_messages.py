"""Generated message classes for gkemulticloud version v1.
 ...
 Anthos clusters on Azure provides a way to manage Kubernetes clusters that
run on Azure infrastructure using the Anthos Multi-Cloud API. Combined with
Connect, you can manage Kubernetes clusters on Google Cloud and Azure from the
Google Cloud Console. When you create a cluster with Anthos clusters on Azure,
Google creates the Azure resources needed and brings up a cluster on your
behalf. You can deploy workloads to Anthos clusters on Azure using the API or
the gcloud and kubectl command-line tools.
"""
...
class GkemulticloudProjectsLocationsAwsClustersPatchRequest(_messages.Message):
  ...
  Fields:
    ...
    updateMask: Mask of fields to update. At least one path must be supplied
      ...
      these fields from AwsCluster: * `description`. * `annotations`.
    validateOnly: If set, only validate the request, but do not actually
      update the cluster.
  """

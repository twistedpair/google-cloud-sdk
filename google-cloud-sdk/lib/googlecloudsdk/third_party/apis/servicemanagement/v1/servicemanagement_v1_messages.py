...
class AliasStrategy(_messages.Message):
  """Strategy that associates each instance of service configuration with
  multiple aliases. Backend servers are partitioned to send requests to Google
  Service Control with different aliases which may point to different versions
  of service configurations.  One example of how to gradually rollout a new
  service configuration using this strategy: servers are partitioned to use
  "DAY_1" and "DAY_2" aliases. Day 1      Rollout {       id:
  "example.googleapis.com/rollout_20160206"       alias_strategy: {
  aliases: {           "DAY_1": "example.googleapis.com/20160206"
  "DAY_2": "example.googleapis.com/20160201"         }       }     }  Google
  Service Control will start using the new version of the configuration for
  traffic comfing from "DAY_1" server group.  Day 2      Rollout {       id:
  "example.googleapis.com/rollout_20160206"       alias_strategy: {
  aliases: {           "DAY_1": "example.googleapis.com/20160206"
  "DAY_2": "example.googleapis.com/20160206"         }       }     }

  Messages:
    AliasesValue: Maps aliases to service configuration IDs.

  Fields:
    aliases: Maps aliases to service configuration IDs.
  """

  @encoding.MapUnrecognizedFields('additionalProperties')
  class AliasesValue(_messages.Message):
    """Maps aliases to service configuration IDs.

    Messages:
      AdditionalProperty: An additional property for a AliasesValue object.

    Fields:
      additionalProperties: Additional properties of type AliasesValue
    """

    class AdditionalProperty(_messages.Message):
      """An additional property for a AliasesValue object.
  ...
  aliases = _messages.MessageField('AliasesValue', 1)
...
class ManagedService(_messages.Message):
  ...
  Fields:
    ...
    serviceName: The name of the service. See the [overview](/service-
      management/overview) for naming requirements. This name must match
...
class Rollout(_messages.Message):
  ...
  Fields:
    aliasStrategy: Google Service Control selects service configurations
      associated with the aliases sent from the service.
    ...
    rolloutId: Optional unique identifier of this Rollout. If not specified by
      client, the server will generate one. The generated id will have the
      form of <date><revision number>, where "date" is the create date in ISO
    ...
    serviceSelectionStrategy: The service itself specifies which configuration
      to use.
  ...
  createdBy = _messages.StringField(3)
  deleteServiceStrategy = _messages.MessageField('DeleteServiceStrategy', 4)
  rolloutId = _messages.StringField(5)
  serviceName = _messages.StringField(6)
  serviceSelectionStrategy = _messages.MessageField('ServiceSelectionStrategy', 7)
  status = _messages.EnumField('StatusValueValuesEnum', 8)
  trafficPercentStrategy = _messages.MessageField('TrafficPercentStrategy', 9)
...
class Service(_messages.Message):
  ...
  enums = _messages.MessageField('Enum', 9, repeated=True)
  http = _messages.MessageField('Http', 10)
  ...
  systemParameters = _messages.MessageField('SystemParameters', 19)
  systemTypes = _messages.MessageField('Type', 20, repeated=True)
  title = _messages.StringField(21)
  types = _messages.MessageField('Type', 22, repeated=True)
  usage = _messages.MessageField('Usage', 23)
  visibility = _messages.MessageField('Visibility', 24)
...
class ServiceSelectionStrategy(_messages.Message):
  """Strategy that lets the service choose which service configuration ID to
  use at runtime from the given collection of IDs.  One example of how to
  gradually rollout a new service configuration using this strategy: Before
  rollout, all servers are sending traffic to the Google Service Control with
  configuration id `example.googleapis.com/20160201`, and Google Service
  Control uses the version of service configuration whose ID is the same as
  the specified request id.  Day 1 The service producer can push this Rollout:
  Rollout {       id: "example.googleapis.com/rollout_20160206"
  client_selection_strategy: {         service_config_ids:
  ["example.googleapis.com/20160201",
  "example.googleapis.com/20160206"]       }     }  Then deploy some of the
  servers so that they can start sending traffic with service configuration id
  `example.googleapis.com/20160206`. And the other servers are still sending
  traffic with service configuration id `example.googleapis.com/20160201`.
  Day 2 Deploy all servers to send traffic with service configuration id
  `example.googleapis.com/20160206`.

  Fields:
    serviceConfigIds: List of service configuration IDs that the service can
      select from.
  """

  serviceConfigIds = _messages.StringField(1, repeated=True)

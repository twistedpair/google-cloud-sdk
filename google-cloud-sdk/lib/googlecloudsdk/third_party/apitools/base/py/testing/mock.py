...
import googlecloudsdk.third_party.apitools.base.py as apitools_base
...
class ExpectedRequestsException(Error):
  ...
  def __init__(self, expected_calls):
    ...
    for (key, request) in expected_calls:
      msg += '{key}({request})\n'.format(
          key=key, request=apitools_base.MessageToRepr(request, multiline=True))
...
class _ExpectedRequestResponse(object):
  ...
  def ValidateAndRespond(self, key, request):
    """Validate that the key and request match expectations, and if so, respond.
    ...
    Raises:
      UnexpectedRequestException: If key or request dont match expectations.
      apitools_base.Error: If a non-None exception is specified to be thrown.
    ...
    if self.__exception:
      # pylint:disable=raising-bad-type, Can only throw apitools_base.Error.
...
class _MockedService(apitools_base.BaseApiService):
  ...
  def __init__(self, key, mocked_client, methods, real_service):
    ...
    for method in methods:
      ...
      setattr(self, method,
              _MockedMethod(key + '.' + method, mocked_client, real_method))
...
class _MockedMethod(object):
  ...
  def Expect(self, request, response=None, exception=None, **unused_kwargs):
    ...
    Args:
      ...
      response: The response that should be returned or None if exception is
          provided.
      exception: An exception that should be thrown, or None.
    """
    # TODO(user): the unused_kwargs provides a placeholder for future things
    # that can be passed to Expect(), like special params to the method call.
  ...
  def __call__(self, request, **unused_kwargs):
    # TODO(user): allow the testing code to expect certain values in these
    # currently unused_kwargs, especially the upload parameter used by
...
class Client(object):
  ...
  def Mock(self):
    ...
    for name in dir(self.__client_class):
      ...
      # pylint:disable=protected-access, Some liberty is allowed with mocking.
      collection_name = service_class._NAME
      api_name = (self.__client_class._PACKAGE
                  + '_' + self.__client_class._URL_VERSION)

...
def DownloadProgressPrinter(response, download):  # pylint: disable=unused-argument
...
def DownloadCompletePrinter(response, download):  # pylint: disable=unused-argument
  """Print information about a completed download."""
  print('Download complete')


def UploadProgressPrinter(response, upload):  # pylint: disable=unused-argument
  """Print upload progress based on response."""
  print('Sent %s' % response.info['range'])


def UploadCompletePrinter(response, upload):  # pylint: disable=unused-argument
...
class Upload(_Transfer):
  ...
  def __SendChunk(self, start, additional_headers=None):
    ...
    if self.total_size is None:
      ...
      # TODO(craigcitro): Here, change body_stream from a stream to a
      # string object, which means reading a chunk into memory. This
      # works around
      # https://github.com/jcgregorio/httplib2/issues/176, which can
      # cause httplib2 to skip bytes on 401's for file objects. Rework
    ...
    if no_log_body:
      ...
      #
      # TODO(craigcitro): Remove no_log_body and rework as part of a
      # larger logs refactor.

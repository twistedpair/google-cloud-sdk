...
class UniverseDescriptorFetchError(exceptions.Error):
...
class UniverseDescriptorDataError(exceptions.Error):
...
def _GetAllConfigUniverseDomains() -> Set[str]:
...
class UniverseDescriptor:
  ...
  def UpdateDescriptorFromUniverseDomain(self, universe_domain: str) -> None:
